...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure
    .mutation(async({ctx}) => {
      ...
      if (!accountLink.url) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Account link failed" })
      }
  ...
  getProducts: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (data.totalDocs !== input.ids.length) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Some products were not found" })
      }
    ...
    .mutation(async ({ctx, input}) => {
      ...
      const platformFeeAmount = Math.round(totalAmount * (PLASTFORM_FEE_PERCENTAGE / 100))
